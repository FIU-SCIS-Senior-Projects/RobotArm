...
#=========================================================================================
    ...
    def onTimeUpdate(self, event):
        ...
        if self.connectFlag & self.is_update & ((self.updateCounter % 5) == 0):
            ...
            try: 
                parent_bone = self.GetParent().bone_list["R_Hand"]
                child_bone = self.GetParent().bone_list["R_Lower_Arm"]
                print "right wrist"
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                r_wrist_x, r_wrist_y, r_wrist_z = joint_vaule
                ...
                if normalized_z > WRIST_ROLL_RANGE:
                    ...
                    self.r_wrist_z_low = r_wrist_z - WRIST_ROLL_RANGE
                print "r_wrist-z-low", self.r_wrist_z_low
                print "normalized z:", normalized_z
                #forearm
                ...
                if normalized_x > FOREARM_YAW_RANGE:
                    ...
                    self.r_wrist_x_low = r_wrist_x - FOREARM_YAW_RANGE

                output = self.gbs.FindItemAtPosition((11, 3)).GetWindow()

                tmp_str = 'RH: {:0.1f} {:0.1f} {:0.1f}'.format(normalized_x, 0, normalized_z)
                if self.connectFlag is True:
                    try:
                        msg_str = '<right_wrist {:d} {:d} {:d}>\n'.format(int(normalized_x), 0, int(normalized_z))
            ...
            try: 
                ...
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                r_elbow_x, r_elbow_y, r_elbow_z = joint_vaule
                ...
                #arm yaw
                normalized_x = r_elbow_x - self.r_elbow_x_low
                if normalized_x < 0:
                    normalized_x = 0
                    self.r_elbow_x_low = r_elbow_x
                if normalized_x > ARM_YAW_RANGE:
                    normalized_x = ARM_YAW_RANGE
                    self.r_elbow_x_low = r_elbow_x - ARM_YAW_RANGE

                output = self.gbs.FindItemAtPosition((12, 3)).GetWindow()

                tmp_str = 'RE: {:0.1f} {:0.1f} {:0.1f}'.format(normalized_x, normalized_y, 0)
                if self.connectFlag is True:
                    try:
                        msg_str = '<right_elbow {:d} {:d} {:d}>\n'.format(int(normalized_x), int(normalized_y), 0)
            ...
            try: 
                ...
                child_bone = self.GetParent().bone_list["R_Shoulder"]
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                r_shoulder_x, r_shoulder_y, r_shoulder_z = joint_vaule
                s_x, s_y, s_z = joint_vaule
                output = self.gbs.FindItemAtPosition((15, 3)).GetWindow()
                vShoulderX = vector(1, 0, 0)
                vShoulderX = rotate(vShoulderX, radians(r_shoulder_x), vX)
                vShoulderX = rotate(vShoulderX, radians(r_shoulder_y), vY)
                vShoulderX = rotate(vShoulderX, radians(r_shoulder_z), vZ)
                ...
                r_shoulder_x = degrees(angleShoulderX)
                r_shoulder_y = degrees(angleShoulderY)
                ...
                if normalized_y > SHOULDER_ROLL_RANGE:
                    normalized_y = SHOULDER_ROLL_RANGE
                    self.r_shoulder_y_low = r_shoulder_y - SHOULDER_ROLL_RANGE
                ...
                if normalized_x > SHOULDER_PITCH_RANGE:
                    normalized_x = SHOULDER_PITCH_RANGE
                    self.r_shoulder_x_low = r_shoulder_x - SHOULDER_PITCH_RANGE

                tmp_str = 'RS: {:0.1f}({:0.1f}:{:0.1f}) {:0.1f}({:0.1f}:{:0.1f}) {:0.1f} '.format(normalized_x, angleShoulderX, s_x, normalized_y, angleShoulderY, s_y , s_z)
                if self.connectFlag is True:
                    try:
            ...
            try:
                parent_bone = self.GetParent().bone_list["L_Hand"]
                child_bone = self.GetParent().bone_list["L_Lower_Arm"]
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                l_wrist_x, l_wrist_y, l_wrist_z = joint_vaule
                ...
                if normalized_z > WRIST_ROLL_RANGE:
                    normalized_z = WRIST_ROLL_RANGE
                    self.l_wrist_z_low = l_wrist_z - WRIST_ROLL_RANGE
                ...
                if normalized_x > FOREARM_YAW_RANGE:
                    normalized_x = FOREARM_YAW_RANGE
                    self.l_wrist_x_low = l_wrist_x - FOREARM_YAW_RANGE
                
                output = self.gbs.FindItemAtPosition((3, 3)).GetWindow()

                tmp_str = 'LH: {:0.1f} {:0.1f} {:0.1f}'.format(normalized_x, 0, normalized_z)
                if self.connectFlag is True:
                    try:
            ...
            try: 
                ...
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                l_elbow_x, l_elbow_y, l_elbow_z = joint_vaule
                ...
                #arm yaw
                normalized_x = l_elbow_x - self.l_elbow_x_low
                if normalized_x < 0:
                    normalized_x = 0
                    self.l_elbow_x_low = l_elbow_x
                if normalized_x > ARM_YAW_RANGE:
                    normalized_x = ARM_YAW_RANGE
                    self.l_elbow_x_low = l_elbow_x - ARM_YAW_RANGE
                
                output = self.gbs.FindItemAtPosition((4, 3)).GetWindow()
              
                tmp_str = 'LE: {:0.1f} {:0.1f} {:0.1f}'.format(normalized_x, normalized_y, 0)
                if self.connectFlag is True:
                    try:
            ...
            try: 
                ...
                child_bone = self.GetParent().bone_list["L_Shoulder"]
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                l_shoulder_x, l_shoulder_y, l_shoulder_z = joint_vaule
                s_x, s_y, s_z = joint_vaule
                output = self.gbs.FindItemAtPosition((7, 3)).GetWindow()

                vShoulderX = vector(1, 0, 0)
                vShoulderX = rotate(vShoulderX, radians(l_shoulder_x), vX)
                vShoulderX = rotate(vShoulderX, radians(l_shoulder_y), vY)
                vShoulderX = rotate(vShoulderX, radians(l_shoulder_z), vZ)
                tmpShoulderX = vector(0, vShoulderX.y, vShoulderX.z)
                angleShoulderX = diff_angle(vY, tmpShoulderX)
                angleShoulderY = diff_angle(vX, vShoulderX)
                ...
                l_shoulder_x = degrees(angleShoulderX)
                l_shoulder_y = degrees(angleShoulderY)
                ...
                if normalized_y > SHOULDER_ROLL_RANGE:
                    normalized_y = SHOULDER_ROLL_RANGE
                    self.l_shoulder_y_low = l_shoulder_y - SHOULDER_ROLL_RANGE
                ...
                if normalized_x > SHOULDER_PITCH_RANGE:
                    normalized_x = SHOULDER_PITCH_RANGE
                    self.l_shoulder_x_low = l_shoulder_x - SHOULDER_PITCH_RANGE

                tmp_str = 'LS: {:0.1f}({:0.1f}:{:0.1f}) {:0.1f}({:0.1f}:{:0.1f}) '.format(normalized_x, angleShoulderX, s_x, normalized_y, angleShoulderY, s_y)
                if self.connectFlag is True:
                    try:
            ...
            try: 
                parent_bone = self.GetParent().bone_list["Head"]
                child_bone = self.GetParent().bone_list["Neck"]
                timestamp, joint_vaule = anim_utils.calculateJointVaule(parent_bone, child_bone, 1)
                neck_x, neck_y, neck_z = joint_vaule

                #head yaw
                normalized_y = neck_y - self.head_y_low
                if normalized_y < 0:
                    normalized_y = 0
                    self.head_y_low = neck_y
                if normalized_y > HEAD_PAN_RANGE:
                    normalized_y = HEAD_PAN_RANGE
                    self.head_y_low = neck_y - HEAD_PAN_RANGE

                #head pitch
                normalized_x = neck_x - self.head_x_low
                if normalized_x < 0:
                    normalized_x = 0
                    self.head_x_low = neck_x
                if normalized_x > HEAD_TILT_RANGE:
                    normalized_x = HEAD_TILT_RANGE
                    self.head_x_low = neck_x - HEAD_TILT_RANGE

                output = self.gbs.FindItemAtPosition((1, 3)).GetWindow()
