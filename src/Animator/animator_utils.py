...
NAMES_3DSMAX = 1
NAMES_CLINIC = 2
MALE_BONE_RATIOS = {"Hips":[0.086, Matrix4(), Vector3(),
...
FEMALE_BONE_RATIOS = {"Hips":[0.085, Matrix4(), Vector3(),
...
NAME_LIST = ['Head', 'Neck', 'Chest', 'Hips', 'L_Shoulder', 'R_Shoulder',
                'L_Upper_Arm', 'L_Lower_Arm', 'L_Hand', 'R_Upper_Arm',
                'R_Lower_Arm', 'R_Hand', 'L_Upper_Leg', 'L_Lower_Leg', 'L_Foot',
...
LEFT_SHOULDER_LIST = ['l_shoulder', 'l_clavicle', 'l_collar', 'l_scapula', 'lshoulder', 'lclavicle', 'lcollar', 'lscapula', 'left_shoulder', 'left_clavicle', 'left_collar', 'left_scapula', 'leftshoulder', 'leftclavicle', 'leftcollar', 'leftscapula']
LEFT_UP_ARM_LIST = ['l_upper_arm', 'l_up_arm', 'l_u_arm', 'l_humerus', 'lupperarm', 'luparm', 'luarm', 'lhumerus', 'left_upper_arm', 'left_up_arm', 'left_u_arm', 'left_humerus', 'leftupperarm', 'leftuparm', 'leftuarm', 'lefthumerus']
LEFT_LOW_ARM_LIST = ['l_lower_arm', 'l_low_arm', 'l_l_arm', 'l_ulna', 'l_radius', 'llowerarm', 'llowarm', 'llarm', 'lulna', 'lradius', 'left_lower_arm', 'left_low_arm', 'left_l_arm', 'left_ulna', 'left_radius', 'leftlowerarm', 'leftlowarm', 'leftlarm', 'leftulna', 'leftradius']
LEFT_HAND_LIST = ['l_hand', 'lhand', 'left_hand', 'lefthand']
LEFT_UP_LEG_LIST = ['l_upper_leg', 'l_up_leg', 'l_u_leg', 'l_femur', 'l_thigh', 'lupperleg', 'lupleg', 'luleg', 'lfemur', 'lthigh', 'left_upper_leg', 'left_up_leg', 'left_u_leg', 'left_femur', 'left_thigh', 'leftupperleg', 'leftupleg', 'leftuleg', 'leftfemur', 'leftthigh']
LEFT_LOW_LEG_LIST = ['l_lower_leg', 'l_low_leg', 'l_l_leg', 'l_tibia', 'l_fibula', 'l_shin', 'llowerleg', 'llowleg', 'llleg', 'ltibia', 'lfibula', 'lshin', 'left_lower_leg', 'leftlow_leg', 'left_l_leg', 'left_tibia', 'left_fibula', 'left_shin', 'leftlowerleg', 'leftlowleg', 'leftlleg', 'lefttibia', 'leftfibula', 'leftshin']
LEFT_FOOT_LIST = ['l_foot', 'lfoot', 'left_foot', 'leftfoot']
RIGHT_SHOULDER_LIST = ['r_shoulder', 'r_clavicle', 'r_collar', 'r_scapula', 'rshoulder', 'rclavicle', 'rcollar', 'rscapula', 'right_shoulder', 'right_clavicle', 'right_collar', 'right_scapula', 'rightshoulder', 'rightclavicle', 'rightcollar', 'rightscapula']
RIGHT_UP_ARM_LIST = ['r_upper_arm', 'r_up_arm', 'r_u_arm', 'r_humerus', 'rupperarm', 'ruparm', 'ruarm', 'rhumerus', 'right_upper_arm', 'right_up_arm', 'right_u_arm', 'right_humerus', 'rightupperarm', 'rightuparm', 'rightuarm', 'righthumerus']
RIGHT_LOW_ARM_LIST = ['r_lower_arm', 'r_low_arm', 'r_l_arm', 'r_ulna', 'r_radius', 'rlowerarm', 'rlowarm', 'rlarm', 'rulna', 'rradius', 'right_lower_arm', 'right_low_arm', 'right_l_arm', 'right_ulna', 'right_radius', 'rightlowerarm', 'rightlowarm', 'rightlarm', 'rightulna', 'rightradius']
RIGHT_HAND_LIST = ['r_hand', 'rhand', 'right_hand', 'righthand']
RIGHT_UP_LEG_LIST = ['r_upper_leg', 'r_up_leg', 'r_u_leg', 'r_femur', 'r_thigh', 'rupperleg', 'rupleg', 'ruleg', 'rfemur', 'rthigh', 'right_upper_leg', 'right_up_leg', 'right_u_leg', 'right_femur', 'right_thigh', 'rightupperleg', 'rightupleg', 'rightuleg', 'rightfemur', 'rightthigh']
RIGHT_LOW_LEG_LIST = ['r_lower_leg', 'r_low_leg', 'r_l_leg', 'r_tibia', 'r_fibula', 'r_shin', 'rlowerleg', 'rlowleg', 'rlleg', 'rtibia', 'rfibula', 'rshin', 'right_lower_leg', 'rightlow_leg', 'right_l_leg', 'right_tibia', 'right_fibula', 'right_shin', 'rightlowerleg', 'rightlowleg', 'rightlleg', 'righttibia', 'rightfibula', 'rightshin']
...
class KeyframeStruct(object):
    __slots__ = ('end_frame_data', 'end_frame_num', 'start_frame_data',
                    'start_frame_num')
...
class RecordSession(object): # A storage class
    ...
    Attributes:
        bone_frame_counter: A dictionary containing the frame number of a bone.
        capture_rate: A float that denotes the rate the data is captured.
        dups: A dictionary containing the number of duplicated data for a bone.
        ...
        keyframe_data: A dictionary containing a list of keyframes.
            For example:
                keyframe_data[BoneName] = [KeyframeStruct, KeyframeStruct, ...]
        name: A string that has the name of the session.
        recorded_bone_list: A dictionary containing BoneState objects of what
            Bone objects were recorded.
    ...
    def recordFrame(self, root, b_list, total_frames):
        ...
        node_list = base_ng.global_input_nodes + base_ng.global_static_nodes
        synched_data = {}
        
        for node in base_ng.global_input_nodes:
            if node.output_ports[0].connection is not None:
                if node.ts_sensor is not None:
                    sensor = node.ts_sensor
                    ...
                    prev_orient = None
                    frame = 0
                    for data in sensor.stream_data:
                        ...
                        if first_frame_timestamp is None:
                            ...
                            synched_data[serial_num].append(
                                [prev_frame_timestamp, quat])
                            frame += 1
                        else:
                            diff_timestamp = ((timestamp -
                                prev_frame_timestamp) * 0.000001)
                            if diff_timestamp >= self.capture_rate:
                                if frame > total_frames:
                                    break
                                frames_passed = int(round(
                                    diff_timestamp / self.capture_rate))
                                if frames_passed <= 1:
                                    cur_quat = Quaternion(tmp_orient)
                                    
                                    prev_quat = Quaternion(prev_orient)
                                    prev_diff_timestamp = ((prev_timestamp -
                                        prev_frame_timestamp) * 0.000001)
                                    
                                    interp_percentage = ((self.capture_rate -
                                        prev_diff_timestamp) / (diff_timestamp -
                                            prev_diff_timestamp))
                                    
                                    prev_frame_timestamp = (((timestamp -
                                        prev_timestamp) * interp_percentage) +
                                        prev_timestamp)
                                    ...
                                    if(self.interp_method ==
                                        ...
                                        interp_orient = prev_quat.slerp(
                                            cur_quat, interp_percentage)
                                        quat = interp_orient
                                    else:
                                    ...
                                    for i in range(frames_passed):
                                        ...
                                        prev_frame_timestamp = (((timestamp -
                                            prev_timestamp) *
                                            interp_percentage) + prev_timestamp)
        ...
        for s in synched_data:
            if len(synched_data[s]) < total_frames:
                for i in range(total_frames - len(synched_data[s])):
                    synched_data[s].append(synched_data[s][-1][:])
        
        frame = 0
        while frame < total_frames:
            self.frame_data.append({})
            for node in node_list:
                if type(node) is base_ng.TSSNode:
                    node.setUpdateData(synched_data)
                    if node.output_ports[0].connection is not None:
                        node.output_ports[0].connection.node.performOperation()
                else:
            ...
            for b in b_list:
                if b.vs_node is not None:
                    ...
                    if frame == 0:
                        ...
                        self.keyframe_data[b.name][-1].end_frame_data = \
                            [tmp_orient]
        ...
        for b in b_list:
            ...
            if b.vs_node is not None:
                ...
                if data is not None:
                    ...
                    # first frame.
                    sensor = b.vs_node.getTSSensor(check_connect=False)
                    if sensor is not None:
                        id_hex = sensor.serial_number_hex
                        if id_hex not in self.first_timestamp:
                            self.first_timestamp[id_hex] = timestamp
                            frame = 0
                        else:
                            diff_timestamp = (timestamp -
                                self.first_timestamp[id_hex])
                            frame = int(round((diff_timestamp *
                                timestamp_units) / self.capture_rate))
            ...
            key_idx = len(self.keyframe_data[b.name]) - 1
            while(self.keyframe_data[b.name][key_idx].start_frame_num >
                    cur_frame):
                key_idx -= 1
        ...
        for frame in range(number_frames):
            ...
            for skel in skeletons:
                for bone in skel.bones:
                    ...
                    key_data = KeyframeStruct()
                    key_data.start_frame_num = frame
                    key_data.start_frame_data = [bone.frames[frame].orient,
                                                    bone.frames[frame].position]
    ...
    def interpolateData(self, bone_name, cur_frame):
        ...
        if self.interp_method == RecordSession.SLERP:
            if keyframe.end_frame_num is not None:
                interp_percentage = ((cur_frame - keyframe.start_frame_num) /
                    (1.0 * (keyframe.end_frame_num - keyframe.start_frame_num)))
                ...
                if interp_percentage > 1:
                    ...
                    interp_percentage = 0
                interp_orient = keyframe.start_frame_data[0].slerp(
                    keyframe.end_frame_data[0], interp_percentage)
                frame_diff = (keyframe.end_frame_data[1] -
                    keyframe.start_frame_data[1])
                interp_pos = (keyframe.start_frame_data[1] + frame_diff *
                    interp_percentage)
...
class Recorder(threading.Thread): # Our threaded recording class
    ...
    def __init__(self, capture_rate, root, b_list, ped_track, log_win):
        ...
        threading.Thread.__init__(self)
        self.capture_rate = capture_rate
        self.session = RecordSession(str(datetime.datetime.today())[:-7].replace(':', '.'),
            capture_rate, ped_track, log_win)
...
class BoneState(object): # Actually a struct
    ...
    Attributes:
        ...
        pose_orient: An instance of Matrix4 that denotes the initial orientation
            of the Bone object.
        sensor_pose: An instance of Euler that denotes the orientation of the
            sensor. Used for TSH Version 0.2 or earlier.
        vs_name: A string that denotes the name of a VirtualSensorNode object.
        vs_pose: An instance of Quaternion that denotes the initial orientation
            of the sensor. Used for TSH Version 0.2 or earlier.
    """
    
    __slots__ = ('bone_length', 'children', 'offset', 'parent', 'pose_orient',
                    'sensor_pose', 'vs_name', 'vs_pose')
...
class SkelNode(object):
    ...
    def getNode(self, node):
        """A function that finds a Bone object in the skeleton tree recursively.
    ...
    def update(self, streaming=False, frame_data=None):
        ...
        Args:
            streaming: A boolean that indicates if the Mocap Studio is streaming
...
class Skeleton(SkelNode):
    ...
    Attributes:
        ...
        foot_epsilon: A float denoting the margin of error for the feet touching
            the ground.
        ...
        low_arm_bone_l: A reference to a Bone object, used for the Left Lower Arm.
        low_arm_bone_r: A reference to a Bone object, used for the Right Lower Arm.
        low_leg_bone_l: A reference to a Bone object, used for the Left Lower Leg.
        low_leg_bone_r: A reference to a Bone object, used for the Right Lower Leg.
        ...
        orientation: An instance of Matrix4 denoting the initial orientation.
        ...
        shoulder_bone_l: A reference to a Bone object, used for the Left Shoulder.
        shoulder_bone_r: A reference to a Bone object, used for the Right Shoulder.
        up_arm_bone_l: A reference to a Bone object, used for the Left Upper Arm.
        up_arm_bone_r: A reference to a Bone object, used for the Right Upper Arm.
        up_leg_bone_l: A reference to a Bone object, used for the Left Upper Leg.
        up_leg_bone_r: A reference to a Bone object, used for the Right Upper Leg.
    """
    
    __slots__ = ('call_lock', 'chest_bone', 'foot_bone_l', 'foot_bone_r',
                    'foot_down_l', 'foot_down_r', 'foot_epsilon','foot_pos_l',
                    'foot_pos_r', 'hand_bone_l', 'hand_bone_r', 'head_bone',
                    'hip_bone', 'low_arm_bone_l', 'low_arm_bone_r',
                    'low_leg_bone_l', 'low_leg_bone_r', 'lowest_point', 'mesh',
                    'neck_bone', 'pose_orientation', 'position',
                    'shoulder_bone_l', 'shoulder_bone_r', 'up_arm_bone_l',
                    'up_arm_bone_r', 'up_leg_bone_l', 'up_leg_bone_r')
    ...
    def buildSkeleton(self, bone_name, height, bone_map, bone_list, canvas,
        ...
        if len(bone_map[bone_name]) > 3:
            for child in bone_map[bone_name][3]:
                self.buildSkeleton(child, height, bone_map[bone_name][3],
                    bone_list, canvas, properties, tmp_bone)
    
    def buildSkeletonCopy(self, skel, bone_list, canvas, parent=None,
                                                            parent_copy=None):
        # Create Bone
        tmp_bone = None
        if parent is None:
    ...
    def lowestPointOffest(self):
        ...
        if self.foot_bone_l is not None:
            ...
            if (bone_pos.y >= (self.lowest_point - self.foot_epsilon) and
                    bone_pos.y <= (self.lowest_point + self.foot_epsilon)):
    ...
    def resetPedTracking(self):
        ...
        self.call_lock.acquire()
        self.hip_bone.setOffset(self.position.copy())
        self.call_lock.release()
        self.update()
...
class Bone(SkelNode):
    ...
    Attributes:
        ...
        mesh_ambient: A tuple denoting the ambient color of the BoneMeshNode.
        name: A string denoting the name of the Bone.
        offset: An instance of Vector3 denoting the offset from parent's
            position.
        orientation: An instance of Matrix4 denoting the current orientation.
    ...
    __slots__ = ('call_lock', 'length', 'mesh', 'mesh_ambient', 'name',
                    'offset', 'orientation', 'pose_orientation', 'position',
                    'skel_orient', 'vs_node')
    ...
    def __init__(self, name, length=5, pos=None, p_orient=None, vs_node=None):
        ...
        Args:
            name: A string denoting a name for the object.
            length: A float that denotes the length of the object (default is 5)
            pos: An instance of Vector3 denoting the absoulte position
                (default is None)
            p_orient: An instance of Matrix4 denoting the initial orientation
                (default is None)
    ...
    def findLowestPoint(self, lowest_point):
        self.call_lock.acquire()
        bone_pos = self.position
        self.call_lock.release()
        # Update My Position
        if type(self.parent) is Bone:
            self.parent.call_lock.acquire()
            par_pos = self.parent.position
            par_orient = self.parent.orientation
            self.parent.call_lock.release()
            self.position = par_pos + (par_orient * self.offset)
        
        if bone_pos.y < lowest_point:
            lowest_point = bone_pos.y
    ...
    def setPoseOrientation(self, pose_orient):
        self.call_lock.acquire()
        self.pose_orientation = pose_orient
        self.call_lock.release()
    ...
    def translate(self, vector):
        ...
        if type(self.parent) is Bone:
            ...
            par_orient = self.parent.orientation
            self.parent.call_lock.release()
            self.position = par_pos + (par_orient * self.skel_orient *
                self.offset)
        elif type(self.parent) is Skeleton:
            self.parent.call_lock.acquire()
            par_pos = self.parent.position
            self.parent.call_lock.release()
            self.position = par_pos + (self.skel_orient * self.offset)
        else:
...
#                print data," = ",tared_orient," $ ",self.orientation
    ...
    def _updateMesh(self):
        # Update Bone Mesh's orientation/position
        self.mesh.orientation = (self.orientation * self.skel_orient *
            self.pose_orientation).asColArray()
        self.mesh.position = self.position.asArray()
#==============================================================        
#        print self.mesh.position," = "
        if DRAW_CHILDREN:
            if type(self.parent) is Bone:
                self.parent.call_lock.acquire()
                par_pos = self.parent.position
                self.parent.call_lock.release()
                tmp_vec = self.position - par_pos
...
#                print "# ",x_vec," # ",y_vec, " # ",z_vec
                ...
                if len(self.mesh.children) > 0:
                    for child in self.mesh.children:
                        ...
                        child.position = par_pos.asArray()
                        child.orientation = tmp_mat
                else:
                    self.mesh.appendChild(gl_sg.GluCylinderNode(0.1, tmp_len, 8,
                        8, par_pos.asArray(), tmp_mat))
...
#        print "**** ", self.orientation
        ...
        if cur_frame > 0:
            ...
            if type(self.parent) is Skeleton:
                self.parent.call_lock.acquire()
                par_pos = self.parent.position
                self.parent.call_lock.release()
                self.position = par_pos + (self.skel_orient * self.offset)
...
def copyBone(bone, selected_list, name_list, parent=None):
    ...
    bone.setAmbientColor((0.2, 0.2, 0.2, 1.0))
    i = 0
